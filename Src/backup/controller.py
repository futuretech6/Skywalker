...
class MouseClass(object):
    ...
    def __init__(self, mouse_speed, ship_speed):
    ...
    def update_view(self):
        ...
        look_here = np.add(self.pos, self.direction)

        look_at = (self.pos[0], self.pos[1], self.pos[2],
    ...
    def move_player(self, delta_time):
        ...
        self.direction = np.multiply(
            self.direction, delta_time * self.ship_speed)
        self.right = np.multiply(self.right, delta_time * self.ship_speed)

        if keys[pygame.K_w]:
            # zbroji trenutnu poziciju i vektor smjera
            self.pos = np.add(self.pos, self.direction)
        if keys[pygame.K_s]:
            self.pos = np.subtract(self.pos, self.direction)
        if keys[pygame.K_a]:
            self.pos = np.subtract(self.pos, self.right)
        if keys[pygame.K_d]:
            self.pos = np.add(self.pos, self.right)

    def angle(self, width, height, delta_time, mouse_pos):
        """Refreshes angle of view, depends on a sensitivity of a mouse (mouse_speed)"""
        self.hor_angle += self.mouse_speed * \
            delta_time * (width / 2 - mouse_pos[0])
        self.vert_angle += self.mouse_speed * \
            delta_time * (height / 2 - mouse_pos[1])
